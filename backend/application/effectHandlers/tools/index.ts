...
export function registerToolEffectHandlers(registry: EffectHandlerRegistry): void {
  registry.register('tool.run', (effect, env, emit) => {
    ...
    tool
      .execute(args, { fs: env.fs, command: env.command }, { toolCallId: effect.toolCallId, runId: effect.runId, conversationId: effect.conversationId, config: effect.config, workEnvironment: effect.workEnvironment, emit: emitRuntimeEvent })
